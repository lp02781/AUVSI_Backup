...
#include "roboboat/rc_number.h"
#include "roboboat/flight_mode.h"
...
void node_master_cb	(const roboboat::node_master& master);
void override_rc_cb	(const roboboat::override_motor& rc);
void rc_number_cb (const roboboat::rc_number& state);
void flight_mode_cb (const roboboat::flight_mode& state);
void image_process_cb (const roboboat::image_process& image);
void setpoint_cb (const roboboat::setpoint& point);
...
int main(int argc, char **argv)
{
	...
	ros::Subscriber sub_node_master 	= nh.subscribe("/auvsi/node/master", 1, node_master_cb);
	ros::Subscriber sub_override_rc 	= nh.subscribe("/auvsi/override/motor", 1, override_rc_cb);
	ros::Subscriber sub_rc_number 		= nh.subscribe("/auvsi/rc/number", 1, rc_number_cb);
	ros::Subscriber sub_flight_mode 	= nh.subscribe("/auvsi/flight/mode", 1, flight_mode_cb);
	ros::Subscriber sub_image_process 	= nh.subscribe("/auvsi/image/process", 1, image_process_cb);
	ros::Subscriber sub_setpoint 		= nh.subscribe("/auvsi/image/setpoint", 1, setpoint_cb);
	...
	ros::Subscriber sub_pid_x_out 		= nh.subscribe("/auvsi/pid/out", 10, pid_receiver_cb );
...
void rc_number_cb (const roboboat::rc_number& number){
	rc_flag 		= number.rc_number;
}
void flight_mode_cb (const roboboat::flight_mode& flight){
